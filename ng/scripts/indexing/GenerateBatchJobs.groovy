...
for (org in orgs) {
    ...
    case "Lucene":
		...
		execLine="java -server -Djava.awt.headless=true -Xmx2500m  -classpath ${cacheClassPath}  org.genedb.web.mvc.model.PopulateLuceneIndices -o ${org} -i ${baseDir}/output/${org} ${dbname} "
        break;

    case "DTO":
        // execLine="ant -f build-apps.xml -Dconfig=${config} -Dorganism=${org} -Ddir=${baseDir}/output/${org} _PopulateCaches "
		// -XX:+HeapDumpOnOutOfMemoryError
		execLine="java -server -Djava.awt.headless=true -Xmx2500m -classpath ${cacheClassPath} org.genedb.web.mvc.model.PopulateCaches -o ${org} --globalFileRoot ${baseDir}/output/${org} "
        new File("${baseDir}/output/${org}").mkdir()
        break;
